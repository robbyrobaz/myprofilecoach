...
function NodeLabel({ nodeData, index, label }: {
  ...
  return (
    <group ref={ref}>
      <Html center distanceFactor={8} style={{ pointerEvents: 'none', userSelect: 'none', whiteSpace: 'nowrap' }}>
        <div style={{
          fontSize: '9px',
          fontFamily: 'ui-monospace, monospace',
          fontWeight: 500,
          color: `rgba(103, 232, 249, 0.7)`,
          textShadow: '0 0 8px rgba(34, 211, 238, 0.5), 0 0 20px rgba(6, 182, 212, 0.3)',
...
// --- 3D Node Network with intensity + mouse reactivity ---
function NodeNetwork({ mouseRef, intensityRef }: {
  ...
  const lerpedIntensity = useRef(0.2)
  ...
  const nodeData = useMemo(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const r = i < NODE_LABELS.length ? 2.0 + Math.random() * 1.2 : 1.5 + Math.random() * 2.5
  ...
  const edgePairs = useMemo(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      for (let j = i + 1; j < COUNT; j++) {
        if (nodeData[i].distanceTo(nodeData[j]) < 3.2) pairs.push([i, j])
      }
    }
    return pairs
  }, [nodeData])
  ...
  useFrame(({ clock, camera }) => {
    ...
    // Smooth lerp intensity (0.2 = ambient, 1.0 = active)
    ...
    const targetZ = 10 - I * 3 // 10 → 7
    camera.position.z += (targetZ - camera.position.z) * 0.02

    // Rotate group + mouse parallax
    if (groupRef.current) {
      const rotSpeed = 0.04 + I * 0.06 // faster when active
      groupRef.current.rotation.y = t * rotSpeed + lerpedMouse.current.x * 0.3
      groupRef.current.rotation.x = Math.sin(t * 0.03) * 0.08 + lerpedMouse.current.y * 0.15
    }

    // Move nodes — faster when active
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const dist = p.length()
      if (dist > 4.5) { v.multiplyScalar(-1); p.normalize().multiplyScalar(4.4) }
      if (dist < 0.8) { v.multiplyScalar(-1); p.normalize().multiplyScalar(0.9) }
    }

    // Nodes
    if (nodesRef.current) {
      for (let i = 0; i < COUNT; i++) {
        const pulseSpeed = 2.5 + I * 3
        const pulse = 0.8 + Math.sin(t * pulseSpeed + i * 0.7) * 0.4
        const baseScale = i < NODE_LABELS.length ? 0.05 : 0.03
        const scale = (baseScale + pulse * 0.015) * (0.8 + I * 0.4)
        ...
        const brightness = 0.35 + I * 0.35
        if (i < NODE_LABELS.length) {
          color.setHSL(0.52 + Math.sin(t * 1.5 + i) * 0.03, 0.6, brightness + pulse * 0.15)
        } else {
    ...
    // Lines — energy flow scales with intensity
    if (linesRef.current) {
      ...
      for (let e = 0; e < edgePairs.length; e++) {
        ...
        const wave2 = Math.sin(t * (waveSpeed * 0.75) + e * 0.3 + 1.5) * 0.5 + 0.5
        const pulse = Math.max(wave1, wave2)
        const dist = a.distanceTo(b)
        const base = Math.max(0, 1 - dist / 3.2)
        const brightnessScale = 0.08 + I * 0.35
        const brightness = base * (brightnessScale + pulse * (0.15 + I * 0.25))
    ...
    if (particlesRef.current) {
      ...
      const angle = 0.001 + I * 0.002
      const cos = Math.cos(angle), sin = Math.sin(angle)
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        const x = posAttr.getX(i), y = posAttr.getY(i), z = posAttr.getZ(i)
        posAttr.setXYZ(i,
          x * cos - z * sin,
          y + Math.sin(t * 0.4 + i) * (0.001 + I * 0.002),
          x * sin + z * cos,
        )
      }
      posAttr.needsUpdate = true
...
// --- HUD Overlay (progress bar, stats, phases) ---
...
// --- Main exported component — renders in layout, always present ---
export default function JarvisBackground() {
  const { state } = useJarvis()
  const mouseRef = useRef({ x: 0, y: 0 })
  const intensityRef = useRef(0.2)

  // Update intensity ref based on mode (read by Three.js without re-renders)
  useEffect(() => {
    intensityRef.current = state.mode === 'active' ? 1.0 : 0.2
  }, [state.mode])
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        <Suspense fallback={null}>
          <Canvas
            ...
            style={{ background: '#050510', pointerEvents: 'none' }}
          >
      ...
      {/* Vignette — always present, stronger when ambient */}
      <div
        className="fixed inset-0 pointer-events-none transition-opacity duration-1000"
        style={{
          zIndex: 1,
          background: 'radial-gradient(ellipse at center, transparent 20%, rgba(5,5,16,0.85) 100%)',
          opacity: state.mode === 'active' ? 0.5 : 1,
        }}
      />
